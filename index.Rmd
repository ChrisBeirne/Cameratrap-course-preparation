...
**Windows** Click on [this link](https://cran.r-project.org/bin/windows/base/) and then `Download R-4.4.2 for Windows`.

**Mac** Click on [this link](https://cran.r-project.org/bin/macosx/) and then the `R-4.4.2.pkgs` link.

**If you already have R**  please update your R client so that it is on the latest version 4.4.2. Checkout the [installR package](https://www.r-project.org/nosvn/pandoc/installr.html) to this directly from the terminal.
...
### An Earthdata Server account

**IMPORTANT** to access MODIS data (and other great products) rapidly through the `modisfast` package you need to create your own login for the associated Earthdata servers.

**To create your free account go to : <https://urs.earthdata.nasa.gov/>.** and make a note of your credentials for use during the Analysis Covariates chapter. 

### Other packages
Some packages must be compiled from sources other than CRAN.

**Windows users: you must install RTools on your computer using [this link](https://cran.r-project.org/bin/windows/Rtools/).**
...
If you navigate through `data > raw_data` you will see that there are three files. One called `example_data`, one called `your_data` and one called `example_covariates`. They are summarized below:
...
These are some files which we will use as predictors for our camera trap models. I will explain all in due course!
